...
/**
 * TemukanMapPicker
 * Renders the interactive SVG map for selecting or previewing report coordinates.
 * In picker mode, clicks calculate viewBox coordinates (560x220) and emit onMapClick({ x, y }).
 *
 * @param {object} props
 * @param {boolean} props.isPickerMode
 * @param {{x: number, y: number}|null} props.pin
 * @param {boolean} props.hasPin
 * @param {({x: number, y: number}) => void} props.onMapClick
 */
export default function TemukanMapPicker({ isPickerMode, pin, hasPin, onMapClick }) {
  ...
  return (
    <div
      className={`relative select-none rounded-xl ${
        isPickerMode
          ? 'cursor-crosshair focus:outline-hidden focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2'
          : ''
      }`}
      onClick={handleClick}
